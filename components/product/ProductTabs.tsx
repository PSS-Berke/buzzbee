...
export default function ProductTabs({ product }: ProductTabsProps) {
  ...
  // Roving tabindex: arrow keys move between tabs, selection follows focus
    ...
    <div className="mt-20">
      ...
      <div className="flex justify-center mb-12 px-4">
        <div
          ...
          className="flex w-full sm:w-auto sm:inline-flex bg-white/60 rounded-full p-1.5 gap-1 border border-[var(--card-border)]/10"
        >
          {tabs.map((tab, index) => (
            <button
              key={tab.key}
              ...
              className={`
                ...
                ${activeTab === tab.key
                  ? 'bg-white text-navy shadow-sm border border-[var(--card-border)]/20'
      ...
      <div className="max-w-4xl mx-auto">
        {activeTab === 'overview' && (
          ...
          >
            <div className="text-center">
              <h2 className="text-2xl font-[family-name:var(--heading-font)] text-navy mb-4">
            ...
            <div>
              <h3 className="text-sm text-[var(--accent-strong)] font-medium text-center mb-6">
                Features
              </h3>
              <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                {product.features.map((feature, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <div className="flex-shrink-0 w-5 h-5 bg-[var(--accent)]/20 rounded-full flex items-center justify-center mt-0.5">
        ...
        {activeTab === 'materials' && (
          ...
          >
            <div className="text-center">
              <h2 className="text-2xl font-[family-name:var(--heading-font)] text-navy mb-2">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {product.materials.map((material, index) => (
                <div
                  key={index}
                  className="bg-white/80 rounded-3xl p-8 border-2 border-[var(--card-border)]/10 hover:border-[var(--card-border)]/30 hover:shadow-lg hover:shadow-[var(--accent)]/5 transition-all duration-500"
                >
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-semibold text-navy text-lg">{material.name}</h3>
                    <span className="px-3 py-1 bg-[var(--accent)]/20 text-[var(--accent-strong)] text-xs font-medium rounded-full">
            ...
            {product.firmness.length > 0 && <div className="bg-[var(--accent)]/10 rounded-3xl p-8 border-2 border-[var(--card-border)]/20">
              <h3 className="font-semibold text-navy text-center mb-8">Firmness Scale</h3>
              {(() => {
                ...
                return (
                  <div className="max-w-lg mx-auto">
                    ...
                    <div className="relative flex items-center mb-2" style={{ height: '32px' }}>
                      {/* Background track */}
                      <div className="absolute inset-x-0 h-1.5 rounded-full bg-[var(--accent)]/20" style={{ top: '50%', transform: 'translateY(-50%)' }} />
                      ...
                      <div className="relative flex justify-between w-full" style={{ top: '50%', transform: 'translateY(-50%)' }}>
                        {scale.map((level) => (
                          <div
                            key={level}
                            className="w-2 h-2 rounded-full bg-[var(--accent)]/30 border border-[var(--card-border)]/40"
                          />
                        ))}
                      </div>
                      {/* Floating marker */}
                      ...
                      >
                        <div className="w-4 h-4 rounded-full bg-[var(--accent)] border-2 border-white shadow-md shadow-[var(--accent)]/40" />
                      </div>
                    </div>
                    {/* Labels */}
                        ...
                        return (
                          <span
                            ...
                            className={`text-xs text-center leading-tight ${
                              isActive ? 'text-[var(--accent-strong)] font-semibold' : 'text-gray-600'
                            }`}
                            style={{ width: '20%' }}
                          >
                            {level}
                    ...
                    <p className="text-center text-sm text-gray-600 mt-6">
                      This mattress is rated <span className="text-[var(--accent-strong)] font-medium">{product.selectedFirmness}</span>
                    </p>
                  </div>
                );
              })()}
          ...
          >
            ...
            <div className={`grid gap-8 ${product.firmness.length > 0 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2 md:grid-cols-3 max-w-3xl mx-auto'}`}>
              {[
                ...
                <div
                  key={i}
                  className="text-center p-8 bg-white/80 rounded-3xl border-2 border-[var(--card-border)]/10"
                >
                  <div className="w-16 h-16 bg-[var(--accent)]/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
                    <step.icon className="w-8 h-8 text-[var(--accent-strong)]" />
                  </div>
